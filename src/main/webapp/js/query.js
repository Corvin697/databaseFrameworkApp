...
let mongoDbResultHtml = new Array()
let newSqlTableData = new Array()
let sqlResultHtml = ""
const changeEvent = new Event("change")
let changedDocumentKeys = new Array()
let changedDocumentValues = new Array()
 ...
 */
function warningYesButtonClicked() {
    ...
    yesButton.addEventListener("click", (event) => {
        ...
        switch (databaseType) {
            case "mongoDb" :
                ...
                for (let i = 0; i < changedDocumentKeys.length; i++) {
                    if (i !== 0) {
                        keyString = keyString + "$" + changedDocumentKeys[i];
                    } else {
                        keyString = keyString + changedDocumentKeys[i];
                    }
                }

                for (let i = 0; i < changedDocumentValues.length; i++) {
                    if (i !== 0) {
                        valueString = valueString + "$" + changedDocumentValues[i];
                    } else {
                        valueString = valueString + changedDocumentValues[i];
                    }
                }
                ...
                break;
            case "sql" :
                payload = "edit sql," + newSqlTableData.toString()
                newSqlTableData = []
                asyncRequest.send(payload)
 ...
 */
function warningNoButtonClicked() {
...
function collectSqlData() {
    ...
    if (generatedRows !== null && headerRow !== null) {
        ...
        newSqlTableData.push((columnHeaders.length - 3))
        //Push column names in the array
        for (let count = 2; count < columnHeaders.length - 1; count++) {
            newSqlTableData.push(columnHeaders[count].innerText)
        }
        ...
        for (let i = 0; i < generatedRows.length; i++) {
            ...
            for (let j = 1; j < columnHeaders.length - 2; j++) {
                newSqlTableData.push(generatedRows[i].getElementsByTagName("td")[j].innerText)
            }
        }
    }
}
...
function showEmbeddedDocumentButtonClicked() {
    ...
    for (let i = 0; i < showEmbeddedDocumentButtons.length; i++) {
        ...
        showEmbeddedDocumentButton.addEventListener("click", (event) => {
            ...
            let embeddedDocument = embeddedDocuments[shownDocumentNumber][showEmbeddedDocumentButtonId].toString()
